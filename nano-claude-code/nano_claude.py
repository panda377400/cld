...
def main():
    ...
    if args.model:
        ...
        # Support legacy "provider:model" input without breaking
        # model tags like "qwen2.5-coder:3b".
        if "/" not in model_arg and ":" in model_arg:
            provider_hint, remainder = model_arg.split(":", 1)
            if provider_hint in PROVIDERS:
                model_arg = f"{provider_hint}/{remainder}"
